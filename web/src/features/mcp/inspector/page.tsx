...
const DEFAULT_ENDPOINT_PATH =
  (import.meta.env.VITE_MCP_ENDPOINT_PATH as string | undefined) || "/mcp";
...
type CreateInspectorFn = (options: {
  target: HTMLElement;
  endpointUrl: string;
...
export function InspectorPage() {
  ...
  useEffect(() => {
    ...
    const authorization =
      params.get("token") || params.get("authorization") || "";
    ...
    async function loadInspector() {
      ...
      try {
        ...
        if (!cryptoReady.ok) {
          setError(
            cryptoReady.message ??
              "Secure WebCrypto APIs are required for the MCP Inspector."
          );
          return;
        }
        ...
        if (!scriptLoader) {
          throw new Error(
            "Inspector script assets not found. Ensure @modelcontextprotocol/inspector-client is installed."
          );
        }
        ...
        const createInspector =
          inspectorModule.createInspector ?? inspectorModule.default;

        if (typeof createInspector !== "function") {
          throw new Error(
            "createInspector export not found in inspector module"
          );
        }
        ...
        if (!cancelled) {
          setError(
            "Failed to load MCP Inspector. Check console for details or open https://inspector.modelcontextprotocol.io manually."
          );
        }
      } finally {
        if (!cancelled) {
    ...
    <div className="flex h-full min-h-[calc(100vh-8rem)] flex-col bg-background text-foreground">
      <header className="border-b border-border bg-card/80 px-6 py-3 text-sm text-muted-foreground">
        ...
        <span className="mt-1 block">
          Use query parameters ?endpoint=&lt;url&gt; and ?token=&lt;value&gt; to
          override defaults.
        </span>
      </header>
...
function ensureSubtleCryptoDigest(): { ok: boolean; message?: string } {
  ...
  const globalCrypto = window.crypto as
  ...
  if (!globalCrypto) {
    return {
      ok: false,
      message:
        "window.crypto is not available in this environment. Use a modern browser or enable secure context.",
    };
  }
