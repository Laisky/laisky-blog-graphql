 ...
 */
export function addOrUpdateTaskId(
  history: TaskIdEntry[],
  value: string
): TaskIdEntry[] {
  ...
  if (existing) {
    ...
    return history.map((e) =>
      e.value === trimmed ? { ...e, usedAt: Date.now() } : e
    );
  }
...
export function TaskIdSelector({
  ...
  const [history, setHistory] = useState<TaskIdEntry[]>(() =>
    loadTaskIdHistory()
  );
  ...
  const handleTogglePin = useCallback(
    (event: React.MouseEvent, entryId: string) => {
      ...
      setHistory((prev) =>
        prev.map((e) => (e.id === entryId ? { ...e, pinned: !e.pinned } : e))
      );
    },
    []
  );
        ...
        >
          ...
          {sortedEntries.length > filteredEntries.length && (
            <div className="border-t px-2 py-1.5 text-xs text-muted-foreground">
              {sortedEntries.length - filteredEntries.length} more hidden by
              filter
            </div>
          )}
        </div>
