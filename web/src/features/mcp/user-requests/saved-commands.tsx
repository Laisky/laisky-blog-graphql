...
export function SavedCommands({ currentContent, onSelectCommand, onSaveCurrentContent, disabled = false }: SavedCommandsProps) {
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      setCommands((prev) => prev.map((cmd) => (cmd.id === editingId ? normalizeCommand(updated) : cmd)));
      handleCancelEdit();
    } catch (err) {
    ...
    <Card className="border border-border/60 bg-card shadow-sm">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          Store frequently used directives for quick access. Click any saved command to load it into the editor. Commands are synced across
          all your devices.
        </p>
      </CardHeader>
      ...
      {isExpanded && (
        <CardContent className="space-y-4">
          ...
          {showSaveInput && (
            <div className="flex flex-col gap-2 rounded-lg border border-primary/30 bg-primary/5 p-4 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" variant="ghost" size="sm" onClick={handleToggleSaveInput} disabled={isSaving}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </div>
          ...
          {!apiKey ? (
            ...
            <div className="rounded-lg border border-dashed bg-muted/50 px-4 py-6 text-center text-sm text-muted-foreground">
              No saved commands yet. Write a directive above and click &quot;Save current&quot; to create your first template.
            </div>
          ) : (
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="mt-1 text-sm text-muted-foreground line-clamp-2 whitespace-pre-wrap">{command.content}</p>
      </div>

      <div className={cn('flex items-center gap-1 opacity-0 transition-opacity', 'group-hover:opacity-100 focus-within:opacity-100')}>
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
