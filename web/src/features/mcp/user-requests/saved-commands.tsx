...
function fuzzyMatch(
  ...
  for (
    let i = 0;
    i < normalizedTarget.length && queryIdx < normalizedQuery.length;
    i++
  ) {
...
export function SavedCommands({
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      setCommands((prev) =>
        prev.map((cmd) =>
          cmd.id === editingId ? normalizeCommand(updated) : cmd
        )
      );
      handleCancelEdit();
    } catch (err) {
  ...
  const handleReorderCommand = useCallback(
    async (id: string, direction: "up" | "down") => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to reorder commands"
        );
      }
    },
    [apiKey, commands]
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className={cn(
              "transition-colors",
              showSaveInput && "bg-primary/10 border-primary/40"
            )}
          >
        ...
        <p className="text-sm text-muted-foreground">
          Store frequently used directives for quick access. Click any saved
          command to load it into the editor. Commands are synced across all
          your devices.
        </p>
      </CardHeader>
      ...
      {isExpanded && (
        <CardContent className="space-y-4">
          ...
          {showSaveInput && (
            <div className="flex flex-col gap-2 rounded-lg border border-primary/30 bg-primary/5 p-4 animate-in fade-in slide-in-from-top-2 duration-200">
              <p className="text-sm font-medium text-foreground">
                Save current content as a command
              </p>
          ...
          {!apiKey ? (
            ...
            <div className="rounded-lg border border-dashed bg-muted/50 px-4 py-6 text-center text-sm text-muted-foreground">
              No saved commands yet. Write a directive above and click
              &quot;Save current&quot; to create your first template.
            </div>
          ) : (
              ...
              >
                {filteredCommands.map((command, index) => (
                  <SavedCommandItem
                    ...
                    isSelected={
                      searchQuery.length > 0 && index === selectedIndex
                    }
                    dataIndex={index}
                  />
                ))}
    ...
    return (
      <div className="rounded-lg border border-primary/40 bg-primary/5 p-4 space-y-3 animate-in fade-in duration-150">
        <Input
          value={editLabel}
          onChange={(e: ChangeEvent<HTMLInputElement>) =>
            onEditLabelChange(e.target.value)
          }
