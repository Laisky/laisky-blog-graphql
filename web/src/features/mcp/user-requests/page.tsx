...
export function UserRequestsPage() {
    ...
    const handleDeleteAllPendingConfirm = useCallback(async () => {
        ...
        try {
            ...
            setStatus({
                message:
                    error instanceof Error ? error.message : 'Failed to delete pending requests.',
                tone: 'error',
            });
        } finally {
    ...
    const handleDeleteConsumedConfirm = useCallback(async () => {
        ...
        try {
            ...
            setStatus({
                message:
                    error instanceof Error ? error.message : 'Failed to delete consumed requests.',
                tone: 'error',
            });
        } finally {
    ...
    const handleAddToPending = useCallback(
        async (request: UserRequest) => {
            ...
            try {
                ...
                setStatus({
                    message:
                        error instanceof Error ? error.message : 'Failed to re-queue directive.',
                    tone: 'error',
                });
            } finally {
                setIsSubmitting(false);
        ...
        <div className="space-y-8">
            ...
            <Card className="border-2 border-primary/40 bg-primary/5 shadow-md dark:bg-primary/10">
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Draft a new instruction for your AI agent. Requests are delivered in FIFO
                        order (oldest first) when they call <code>get_user_request</code>.
                    </p>
