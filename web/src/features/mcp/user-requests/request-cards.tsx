...
import { ChevronDown, Copy, Edit3, GripVertical, Package, RotateCcw, Trash2, Undo2 } from 'lucide-react';
...
export function EmptyState({ message, subtle = false }: EmptyStateProps) {
  return (
    <div className={cn('rounded-lg border border-dashed px-4 py-6 text-sm text-muted-foreground', subtle ? 'bg-muted/50' : 'bg-muted')}>
      {message}
    </div>
  );
}
      ...
      <CardContent className="flex justify-end gap-2 shrink-0 pt-0">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" disabled={isEditorDisabled} className="gap-1.5" title="Edit options">
              <Edit3 className="h-4 w-4" />
              <ChevronDown className="h-3.5 w-3.5 opacity-70" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem onClick={() => onPickup(request)} className="gap-2 cursor-pointer">
              <Edit3 className="h-4 w-4" />
              <span>Edit & Delete</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onCopy(request)} className="gap-2 cursor-pointer">
    ...
    <Card className="border border-border/60 bg-card min-h-32 max-h-56 flex flex-col w-full min-w-0 transition-all hover:shadow-md hover:border-border">
      ...
      <CardContent className="flex flex-wrap justify-end gap-2 shrink-0 pt-0 pb-3">
        {isPicked ? (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" disabled={isEditorDisabled} className="gap-1.5" title="Pick up this directive">
                <Package className="h-4 w-4" />
                <ChevronDown className="h-3.5 w-3.5 opacity-70" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => onAddToPending(request)} className="gap-2 cursor-pointer">
                <RotateCcw className="h-4 w-4" />
                <span>Add to Pending</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onEditInEditor(request)} className="gap-2 cursor-pointer">
                <Edit3 className="h-4 w-4" />
 ...
 */
export function RequestListSection({ title, badge, emptyMessage, children, isEmpty, subtle = false }: RequestListSectionProps) {
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">{isEmpty ? <EmptyState message={emptyMessage} subtle={subtle} /> : children}</div>
    </div>
  );
}
