...
export function CallLogPage() {
  ...
  const [pagination, setPagination] = useState<
    CallLogListResponse["pagination"] | null
  >(null);
  const [status, setStatus] = useState<string>(
    "Connect with your API key to view call history."
  );
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        ...
        <p className="max-w-2xl text-lg text-muted-foreground">
          Review every MCP tool invocation associated with your bearer token.
          Filter by tool, time range, or API key prefix to monitor usage and
          costs.
        </p>
      </section>
      ...
      <Card className="border border-border/60 bg-card shadow-sm">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Enter the bearer token assigned to your agent. The API key is stored
            locally only.
          </p>
        </CardHeader>
              ...
              {entries.length === 0 ? (
                <tr>
                  ...
                  >
                    {isLoading
                      ? "Loading call logs…"
                      : "No call logs available for the selected filters."}
                  </td>
                </tr>
              ) : (
                entries.map((entry) => (
                  <tr key={entry.id} className="align-top">
                    <td className="px-4 py-3 whitespace-nowrap">
                      {formatTimestamp(entry.occurred_at, dateFormatter)}
                    </td>
        ...
        <div className="flex items-center justify-between border-t border-border/60 px-4 py-3 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              disabled={
                Boolean(totalPages && displayedPage >= totalPages) || isLoading
              }
