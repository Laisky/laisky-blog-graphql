    ...
    <div className="space-y-8">
      ...
      <div className="overflow-hidden rounded-xl border border-border/60 bg-card">
        ...
        <div className="relative overflow-x-auto">
              ...
              {entries.length === 0 ? (
                ...
                entries.map((entry) => (
                  <tr key={entry.id} className="align-top">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">{formatCost(entry.cost_usd)}</td>
          ...
          {isLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-background/70 text-sm text-muted-foreground">Loading…</div>
          )}
        </div>
...
function StatusBadge({ status, error }: { status: string; error?: string | null }) {
  ...
  return (
    <div className="space-y-1">
      ...
      {!isSuccess && error ? <p className="max-w-xs text-xs text-muted-foreground/80">{error}</p> : null}
    </div>
  );
}
