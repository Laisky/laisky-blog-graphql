...
func TestSearcherRunCandidateLimitKeepsLexicalFirstDocuments(t *testing.T) {
	...
	for i, path := range []string{"/z.pdf", "/b.pdf", "/a.pdf"} {
		...
		tree := &Tree{
			...
			Pages:     []Page{{Page: 1, Content: path}},
		}
		require.NoError(t, store.PutTree(ctx, "project", docID, tree))
		require.NoError(t, store.UpdateIndexEntry(ctx, "project", path, IndexEntry{DocID: docID, Type: "pdf"}))
	}
...
func BenchmarkFilterCandidatesLimited100K(b *testing.B) {
	ix := make(Index, 100_000)
	for i := 0; i < 100_000; i++ {
		path := fmt.Sprintf("/docs/%06d.md", 99_999-i)
		ix[path] = IndexEntry{DocID: fmt.Sprintf("doc-%06d", i), Type: "markdown"}
	}
