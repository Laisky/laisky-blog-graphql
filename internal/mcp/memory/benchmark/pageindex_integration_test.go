...
func TestPageIndexPersistsAndSearchesThroughRealSystemFS(t *testing.T) {
	...
	storageProject := pageindexplugin.PageIndexStorageProject(auth, project)
	index, err := store.GetIndex(ctx, storageProject)
	require.NoError(t, err)
	...
	tree, err := store.GetTree(ctx, storageProject, index[documentPath].DocID)
	require.NoError(t, err)
